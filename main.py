...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Стили для полноэкранной мини-карты */
    ...
    .confirm-marker-btn:hover {
      background: var(--primary-dark);
    }
    .confirm-marker-btn:disabled {
      background: var(--background-medium);
      cursor: not-allowed;
      box-shadow: none;
    }
...
<body>
      ...
      <div class="quick-addresses" id="quickAddresses">
        ...
        <div class="quick-address-item" data-address="work">
          ...
          <div class="quick-address-content">
            ...
            <span class="quick-address-subtitle" id="work-subtitle">Не установлена</span>
          </div>
        </div>
  ...
  <!-- Полноэкранный контейнер для мини-карты -->
  <div class="fullscreen-map-container" id="fullscreenMapContainer">
    <div class="fullscreen-map-header">
      <button class="close-fullscreen-map" id="closeFullscreenMap">
        <i class="fas fa-times"></i>
      </button>
      <div class="fullscreen-map-title" id="fullscreenMapTitle">Выберите точку на карте</div>
    ...
    <button class="confirm-marker-btn" id="confirmMarkerBtn" disabled>Подтвердить выбор</button>
  </div>
  ...
  <script>
    ...
    const historyOrdersContainer = document.getElementById('historyOrdersContainer');
        ...
        // Добавляем DOM-элементы для полноэкранной карты в начало кода
    const fullscreenMapContainer = document.getElementById('fullscreenMapContainer');
    const fullscreenMapElement = document.getElementById('fullscreenMap');
    const closeFullscreenMapBtn = document.getElementById('closeFullscreenMap');
    const fullscreenMapTitle = document.getElementById('fullscreenMapTitle');
    ...
    // ======================
    // ФУНКЦИИ ДЛЯ УСТАНОВКИ МАРКЕРА (ПОЛНОЭКРАННЫЙ РЕЖИМ)
    // ======================
    let fullscreenMap = null;
    let fullscreenMarker = null;
    let targetAddressType = null; // 'start' или 'end'
    
// Исправляем функцию setupMarkerSetButtons
function setupMarkerSetButtons() {
  if (setStartMarkerBtn && setEndMarkerBtn) {
    setStartMarkerBtn.addEventListener('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      openFullscreenMap('start');
    });
    
    setEndMarkerBtn.addEventListener('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      openFullscreenMap('end');
    });
  }
  
  // Настраиваем обработчики для кнопок полноэкранной карты
  if (closeFullscreenMapBtn) {
    closeFullscreenMapBtn.addEventListener('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      closeFullscreenMap();
    });
  ...
  if (confirmMarkerBtn) {
    confirmMarkerBtn.addEventListener('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
...
// Исправляем функцию openFullscreenMap
function openFullscreenMap(addressType) {
  targetAddressType = addressType;
  
  // Устанавливаем заголовок
  if (addressType === 'start') {
    fullscreenMapTitle.textContent = 'Выберите точку отправления';
  } else {
    fullscreenMapTitle.textContent = 'Выберите точку назначения';
  }
  ...
  if (!fullscreenMap) {
    ...
    // Обновляем центр карты и позицию маркера при последующих открытиях
    if (targetAddressType === 'start' && startMarker) {
      fullscreenMap.setCenter(startMarker.geometry.getCoordinates(), 15);
      fullscreenMarker.geometry.setCoordinates(startMarker.geometry.getCoordinates());
    } else if (targetAddressType === 'end' && endMarker) {
      fullscreenMap.setCenter(endMarker.geometry.getCoordinates(), 15);
      fullscreenMarker.geometry.setCoordinates(endMarker.geometry.getCoordinates());
    }
    updateConfirmButtonState(true);
  }
  
  // Добавляем обработчик для клавиши Esc
  document.addEventListener('keydown', closeFullscreenMapOnEsc);
}
    ...
    function closeFullscreenMap() {
      ...
      targetAddressType = null;
      
      // Удаляем обработчик Esc
      document.removeEventListener('keydown', closeFullscreenMapOnEsc);
    }
    
// Полностью переписываем функцию initFullscreenMap
function initFullscreenMap() {
  // Получаем текущие координаты для центрирования
  let centerCoords = currentLocation || [56.013263, 90.417379];
  if (targetAddressType === 'start' && startMarker) {
    centerCoords = startMarker.geometry.getCoordinates();
  } else if (targetAddressType === 'end' && endMarker) {
    centerCoords = endMarker.geometry.getCoordinates();
  }
  
  // Создаем карту
  fullscreenMap = new ymaps.Map('fullscreenMap', {
    center: centerCoords,
    zoom: 15,
  ...
  // Создаем ОСНОВНОЙ маркер для выбора (тот самый с черной точкой внизу)
  fullscreenMarker = new ymaps.Placemark(centerCoords, {
    hintContent: 'Выбранная точка'
  }, {
    preset: 'islands#redCircleDotIconWithCaption',
    iconColor: '#FF0000',
    draggable: true // Включаем перетаскивание маркера
  });
  
  fullscreenMap.geoObjects.add(fullscreenMarker);
  
  // Отключаем всплывающие подсказки для POI
  fullscreenMap.behaviors.disable('rightMouseButtonMagnifier');
  
  // Обработчик окончания перетаскивания маркера
  fullscreenMarker.events.add('dragend', function() {
    updateConfirmButtonState(true);
  });
  
  // Обработчик геолокации
  const geolocationControl = fullscreenMap.controls.get('geolocationControl');
  if (geolocationControl) {
    geolocationControl.events.add('locationchange', function(e) {
      const coords = e.get('position');
      if (coords) {
        fullscreenMap.setCenter(coords, 15);
        fullscreenMarker.geometry.setCoordinates(coords);
        updateConfirmButtonState(true);
      }
    });
  }
  
  // Сразу активируем кнопку подтверждения
  updateConfirmButtonState(true);
}
    
    function updateConfirmButtonState(enabled) {
      confirmMarkerBtn.disabled = !enabled;
    }
    
    function closeFullscreenMapOnEsc(e) {
      if (e.key === 'Escape') {
        closeFullscreenMap();
      }
    }
    
// Переписываем функцию confirmMarkerSelection
function confirmMarkerSelection() {
  if (!fullscreenMarker || !targetAddressType) return;
  
  const coords = fullscreenMarker.geometry.getCoordinates();
  
  // Используем геокодирование с правильной областью поиска
  ymaps.geocode(coords, { 
    results: 1,
    boundedBy: currentUserCity.bounds
  }).then(res => {
    const geoObject = res.geoObjects.get(0);
    if (geoObject) {
      const fullAddr = geoObject.getAddressLine();
      
      if (targetAddressType === 'start') {
        startMarker.geometry.setCoordinates(coords);
        startInput.value = fullAddr;
        startInputFull.value = fullAddr;
      } else {
        endMarker.geometry.setCoordinates(coords);
        endInput.value = fullAddr;
        endInputFull.value = fullAddr;
      }
      ...
      // Пересчитываем маршрут, если оба адреса заполнены
      if (startInput.value.trim() && endInput.value.trim()) {
        calculateRoute();
      }
      
      console.log('✅ Адрес успешно установлен:', fullAddr);
    } else {
      console.warn('❌ Не удалось получить геообъект для координат:', coords);
      alert('Не удалось определить адрес для выбранной точки');
      closeFullscreenMap();
    }
  }).catch(error => {
    console.error('❌ Ошибка при геокодировании координат:', error);
    alert('Ошибка при получении адреса. Попробуйте выбрать другую точку.');
    closeFullscreenMap();
  });
}
    ...
    async function showProfileModal() {
      ...
      try {
        ...
        if (profileData.ride_count >= 30) {
          status = "Платина"; statusClass = "stat-platinum";
        } else if (profileData.ride_count >= 20) {
          status = "Золото"; statusClass = "stat-gold";
        } else if (profileData.ride_count >= 10) {
